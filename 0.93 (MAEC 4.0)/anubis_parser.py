...
class parser:
    ...
    def __process_file_activities(self, file_activity, current_process_obj):
        ...
        '''for fs_control in file_activity.get_fs_control_communication():
            ...
            if not is_pipe:
                action_attributes['name'] = {'value' : 'send control code to file', 'xsi:type' : 'maecVocabs:FileActionNameVocab-1.0'}
            else:
                action_attributes['name'] = {'value' : 'send control code to pipe', 'xsi:type' : 'maecVocabs:IPCActionNameVocab-1.0'}
    ...
    def __process_registry_activities(self, registry_activity, current_process_obj):
        ...
        for read_regvalue in registry_activity.get_reg_value_read():
            ...
            actual_key = actual_key.rstrip('\\')
            regkey_attributes['key'] = actual_key
            regkey_attributes['values'] = [{ 'name' : read_regvalue.get_value_name(),
                                             'data' : read_regvalue.get_value_data() }]
            regkey_attributes['xsi:type'] = "WindowsRegistryKeyObjectType"
            regkey_attributes['type'] = 'Key/Key Group'
